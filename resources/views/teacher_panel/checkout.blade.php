					...
					<div class="row">
						...
						<div class="col-6 col-lg-6 col-sm-6 payment-box ">
							<div class="payment-box-inner  card-commission-section  parent-details-section">
								...
								<table width="100%">
									<tr>
										...
										<td class="tbl_hdng" width="25%">Teacher Name</td>
										<td class="tbl_hdng" width="30%">
											@if($aHldy == 1 || $nDelvStatus == 1)
												Pickup Time
											@else
												Order Type
										...
										<td class="tbl_hdng PckTm d-none" width="20%">Pickup Time</td>
									</tr>
									<tr height="10"></tr>
									<tr>
										<td class="tbl_dtl" width="25%">{{ $aTchrDtl['sSchl_Name'] }}</td>
										<td class="tbl_dtl" width="25%">{{ $aTchrDtl['sFrst_Name']}} {{$aTchrDtl['sLst_Name'] }}</td>
										<td class="tbl_dtl" width="30%">
											@if($aHldy == 1 || $nDelvStatus == 1)
												<input type="time" onBlur="getTime(this.value)" class="@error('sPicTm') is-invalid @enderror">
											@else
												<input type="radio" name="nOrdTp" value="910" checked>&nbsp; Delivery&nbsp;
												<input type="radio" name="nOrdTp" value="909">&nbsp; Pickup
											@endif
										</td>
										<td class="tbl_dtl PckTm d-none" width="30%">
											<input type="time" onBlur="getTime(this.value)" class="@error('sPicTm') is-invalid @enderror" style="width: 85px; height: 30px;">
										</td>
									</tr>
								</table>
							</div>
							<div class="payment-box-inner  card-commission-section  parent-details-section">
								...
								<form role="form" action="{{ route('checkout.teacher.stripe') }}" method="post" class="require-validation" data-cc-on-file="false" data-stripe-publishable-key="{{ env('STRIPE_KEY') }}" id="payment-form">
									@csrf
									...
									<div class="row">
										<div class="col-12 p-0">
											<div class="add-btn  mt-0 ml-3">
												<button title="Pay Now" class="mt-0 mt-0 w-100 pay-card" type="submit">
								...
								<form role="form" action="{{ route('checkout.teacher.credit') }}" method="post" class="cr-pay d-none" id="general_form">
									<input class='form-control' type='hidden' name="nOrderType" value="{{ $aHldy == 1 || $nDelvStatus == 1 ? 909 : 910 }}">
									<input class='form-control' type='hidden' name="sPicTm" id="sPicTm">
									@csrf 
									<div class="row">
										<div class="col-12 p-0">
											<div class="add-btn mt-0 ml-3"><button title="Pay Now" class="btn btn-primary pay-button btn-lg btn-block" type="button">Pay Now (${{ number_format($sTtlAmo , 2, '.', '') }})</button></div>
										</div>
									</div>
								</form>	
							</div>
...
$(document).ready(function(){
	$("input[type='radio']").change(function(){
		...
		if($(this).val() == '909')
		{
			$('.PckTm').removeClass('d-none');
		}
		else
		{
			$('.PckTm').addClass('d-none');
		}
	});
});
</script>