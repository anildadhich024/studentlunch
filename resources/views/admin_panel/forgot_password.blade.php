                ...
                <div class="col-12"> 
                    <div class="loginbox m-auto">
                        ...
                        <div class="text-center pt-2 pb-4">
                            Provide email address to retrive password
                        </div>
                        <div class="loginform px-4">
                            <form autocomplete="off" method="post" action="{{url('admin_panel/forgot_password/email')}}" id="general_form">
                                <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                <div class="form-group">
                                    <label for="exampleInputEmail1">Email address</label>
                                    <input type="text" class="form-control @error('sLgnEmail') is-invalid @enderror" name="sLgnEmail" placeholder="Enter email" autocomplete="off" required value="{{old('sLgnEmail')}}" onkeypress="return LenCheck(event, this.value, '50')"> 
                                    @error('sLgnEmail') <div class="invalid-feedback"><span>{{$errors->first('sLgnEmail')}}</span></div>@enderror
                                </div>
                                <button type="submit" class="btn btn-primary mb-4">Send Email</button>
                            </form>
                        </div> 
                    </div>
