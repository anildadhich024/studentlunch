								...
								@if(count($oOrderLst) > 0)
                                    @foreach($oOrderLst As $aRec)
										<tr>
											...
											<td>{{$aRec->nUser_Type == config('constant.USER.CHILD') ? $aRec->sChld_FName : $aRec->sPrnt_FName}}</td>
											...
											<td class="action-btns">
												<ul>
													...
													@if($aRec->nOrdr_Status == config('constant.ORDER_STATUS.Pending') && $aRec->sDelv_Date >= date('Y-m-d'))
													   <li class="detail_btn my-order-btns"><a href="#" data-toggle="modal" data-target="#OrderCancelModel" onclick="CnclOrd('{{base64_encode($aRec->lOrder_IdNo)}}','{{base64_encode($aRec->nUser_Type)}}')">Cancel</a></li>
													@endif
												</ul>
											</td>
										</tr>
...
{ 
    ...
    {
        var Url=APP_URL+"/parent_panel/cancel_order";
        $('#OrderCancelModel #cancelForm').attr('action',Url);
    } 
}
function CnclOrd(lOrderIdNo, nUserType)
{
    if(lOrderIdNo != '')
    { 
        if(atob(nUserType) == '{{config('constant.USER.CHILD')}}')
        {
            $('#OrderCancelModel #OthrRsn').addClass('d-none');
            $('#OrderCancelModel #ChldRsn').removeClass('d-none');
        }
        else
        {
            $('#OrderCancelModel #OthrRsn').removeClass('d-none');
            $('#OrderCancelModel #ChldRsn').addClass('d-none');
        }
